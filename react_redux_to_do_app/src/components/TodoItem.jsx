...
const TodoItem = ({todo, index}) => {
  ...
  return (
    <li className="flex flex-col sm:flex-row sm:items-center justify-between border-b-2 py-2 gap-4">
        ...
        {
          !todo.completed && (
            <button onClick={() => dispatch(markCompleted(index))} className='mr-2 text-sm bg-blue-500 text-white sm:px-2 py-1 px-1 rounded'>
              <FaCheck />
            </button>
          )
        }
        ...
        {
          todo.completed && (
            <button onClick={() => dispatch(markInComplete(index))} className='mr-2 text-sm bg-yellow-500 text-white sm:px-2 py-1 px-1 rounded'>
              <FaTimes />
            </button>
          )
        }
